...
void GpsSerialPlugin::initAndStart(const Json::Value &config)
{
    ...
    if (!isOpen_) {
        LOG_INFO << "The Gps serial port is not open";
        return;
    }
    ...
    asyncSerialPtr_->setReadCallback(std::bind(SerialCallback::gpsReadCallback, std::placeholders::_1, std::placeholders::_2));

    // only read
    asyncSerialPtr_->setWrite(false);

    LOG_INFO << "The Gps serial port is open!"
             << " Port: " << port << " Baudrate: "
...
void GpsSerialPlugin::shutdown()
{
    /// Shutdown the plugin
    asyncSerialPtr_.reset();
    LOG_DEBUG << "GpsSerialPlugin shutdown";
}
